...
class Trainer:
    ...
    def __init__(self, cfg, run_dir, gpu=None, rank=None, world_size=None):
        ...
        #self.model = create_model(cfg, self.train_loader[0].dataset.img_size).to(self.device)
        ...
        #self.n_epoches, self.n_batches = cfg["training"].get("n_epoches"), len(self.train_loader[0])+len(self.train_loader[1])
    ...
    def run_single_batch_train(self, images):
        ...
        for params in self.model.weights_field.parameters():
    ...
    def run_val_and_log(self, it, epoch, batch):
        ...
        scores = self.model.quantitative_eval(self.val_loader, self.device, evaluator=self.val_scores)
        #scores = self.model.quantitative_eval(self.test_loader, self.device,"part")
        print_log(LOG_FMT(epoch, self.n_epoches, batch, self.n_batches,
                          "test_scores: " + ", ".join(["{}={:.4f}".format(k, v) for k, v in scores.items()])))
        ...
        N = 32
        self.model.qualitative_eval(self.val_loader, self.device, path=out, N=N)
        return scores
    ...
    def step(self, epoch, batch):
        ...
        if hasattr(self.train_loader.dataset, 'step'):
            ...
            #self.train_loader[0].dataset.step()
            #self.train_loader[1].dataset.step()
    ...
    def evaluate(self):
        self.model.eval()
        # quantitative
        scores = self.model.quantitative_eval(self.test_loader, self.device, "part_elevator")
