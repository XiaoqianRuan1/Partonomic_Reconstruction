...
class Unicorn(nn.Module):
    ...
    def segment_loss(self,part_pred,part_ground,alpha,masks):
        """
        ...
        ground_fgs = ground_fgs.permute(0,3,1,2)*masks
        part_fgs = part_fgs * alpha
    ...
    #def compute_losses(self, meshes, refined_meshes, alpha, refined_alpha, masks, rec, refined_rec, imgs, rec_part, refined_rec_part, R, T, average_txt=False):
        ...
        if "parts" in losses:
            losses['parts'] = self.loss_weights['parts'] * self.segment_loss(rec_part,parts,alpha,masks).abs().mean()
        ...
        if update_3d and 'neighbor' in losses and (self.tx_code_size > 0 and self.sh_code_size > 0):
            ...
            with torch.no_grad():
                ...
                if self.shared_encoder:
                    ...
                    v,f = m.get_mesh_verts_faces(0)
                    tx_verts = v + self.predict_disp_verts(v,tx_local_features,tx_features).view(B, -1, 3)
    ...
    def quantitative_eval(self, loader, device, evaluator=None):
        ...
        if loader.dataset.name in ['cub_200']:
            ...
            for j, (inp, labels) in enumerate(loader):
                ...
                evaluator.results_output(meshes_part, torch_to(labels, device))
                self.eval()
                self.save_prototype(path / 'model')
                txt_path = os.path.join(path,'results.txt')
                #txt_file = path_mkdir(txt_path)
                qual_path = os.path.join(path,'quali_eval')
                qual_file = path_mkdir(qual_path)
                evaluator.results_output(meshes_part,labels,txt_path)
                ...
                for k in range(B):
                    ...
                    print(i)
                    convert_to_img(imgs[k]).save(qual_file / f'{i}_inpraw.png')
                    convert_to_img(parts[k]).save(qual_file / f'{i}_part.png')
                    mcenter = normalize(meshes[k])
                    mcenter_part = normalize(meshes_part[k])
                    save_mesh_as_obj(mcenter, qual_file / f'{i}_mesh.obj')
                    save_mesh_as_gif(mcenter, qual_file / f'{i}_mesh.gif', dist=d, elev=e, renderer=self.layer_renderer)
                    mcenter.textures = self.get_synthetic_textures(colored=True)
                    save_mesh_as_obj(mcenter, qual_file / f'{i}_meshuv.obj')
                    mcenter_part.textures = self.get_outputs_colors(color_map,k)
                    save_mesh_as_obj(mcenter_part, qual_file / f'{i}_mesh_part_weights.obj')
                    save_mesh_as_gif(mcenter_part, qual_file / f'{i}_mesh_part_weights.gif', dist=d, elev=e, renderer=self.layer_renderer)
