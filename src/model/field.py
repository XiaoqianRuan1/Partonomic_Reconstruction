...
class Field(nn.Module):
    ...
    def __init__(self, n_units=N_UNITS, n_layers=N_LAYERS, latent_size=None, in_ch=3,
                 out_ch=3, zero_last_init=False, with_norm=False, dropout=False, bias_last=True):
        super().__init__()
        NU, NL = n_units, n_layers
        if latent_size is not None:
            self.linear_x = nn.Linear(in_ch, NU) # input channels
            self.linear_z = nn.Linear(latent_size, NU) # the hidden units
            if with_norm:
...
class ProgressiveField(nn.Module):
    def __init__(self, inp_dim, name, powers, zero_last_init=True, milestones=None, **kwargs):
        super().__init__()
        kwargs = deepcopy(kwargs)
        self.powers = [powers] if isinstance(powers, int) else powers
        self.n_powers = len(self.powers) # 2,8,32 the powers of deform field in config files
        ...
        NU, NL = kwargs.pop('n_reg_units', N_UNITS), kwargs.pop('n_reg_layers', N_LAYERS) # NU=128, NL=3;
        bias_last = kwargs.pop('bias_last', True)
        self.regressor = create_mlp(inp_dim, self.latent_size, NU, NL, zero_last_init=zero_last_init)
        NU, NL = kwargs.pop('n_field_units', N_UNITS), kwargs.pop('n_field_layers', N_LAYERS) # NU=18, NL=3;
        self.field = Field(NU, NL, latent_size=self.latent_size, zero_last_init=zero_last_init, bias_last=bias_last)
        self.cur_milestone = 0
        self.set_milestones(milestones)
        assert len(kwargs) == 0
        #self.flip = Variable(torch.oness(1,3).cuda(),requries_grad=False)
        #self.flip[0,0] = -1
    ...
    def symmetrize(self, v):
        if self.symmetric:
            if v.dim() == 2:
                v_left = self.filp * v[-self.num_sym:]
...
class PartProgressiveField(nn.Module):
    def __init__(self,inp_dim,name,K,**kwargs):
        super().__init__()
        self.pro_field = Progressive(inp_dim,name,**kwargs)
        
    def forward(self,x,features,K):
        outputs = []
        for k in range(K):
            outputs.append(self.pro_field(x,features))
...
class TextureField(nn.Module):
    def __init__(self, inp_dim, name, powers, milestones=None, **kwargs):
        ...
        #self.txt_field = Tx_Field(NU, NL, zero_last_init=True, bias_last=bias_last)
        self.cur_milestone = 0
        self.set_milestones(milestones)
        assert len(kwargs) == 0
    ...
    def forward(self, maps):
        #[32,642,32]
        B, N, C, device = maps.size(0), maps.size(1), self.latent_size, maps.device
        latent_final = self.regressor(maps)
        #[32,1024,32]
        if self.act_idx < self.n_powers:
            p = self.current_code_size
            mask = torch.zeros(B, N, C, device=device)
            mask[:, :, :p] = torch.ones(B, N, p, device=device)
            latent_final = mask * latent_final

        self._latent = latent_final
        #[642,3]
        #[32, 642, 32]
        x = latent_final.resize(B, 32, 32, 32)
        x = x.permute(0, 3, 1, 2)
        x = self.transconv1(x)
        x = self.conv1(x)
        x = self.relu1(x)
