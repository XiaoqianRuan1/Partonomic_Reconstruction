...
class Unicorn(nn.Module):
    ...
    def forward(self, inp, debug=False):
        ...
        coarse_meshes, coarse_meshes_part, meshes, meshes_part, (R, T), bkgs = self.predict_mesh_pose_bkg(imgs, average_txt)
        ...
        coarse_meshes_to_render = repeat(coarse_meshes, len(T) // len(coarse_meshes))
        coarse_fgs, coarse_alpha = self.layer_renderer(coarse_meshes_to_render, R, T).split([3, 1], dim=1)  # (K*B)CHW
        coarse_rec = coarse_fgs * coarse_alpha + (1 - coarse_alpha) * bkgs if self.pred_background else coarse_fgs
        coarse_parts_to_render = repeat_part(coarse_meshes_part, len(T) // len(coarse_meshes_part))
        coarse_rec_part = self.part_renderer(coarse_parts_to_render, R, T)
        ...
        #losses, select_idx = self.compute_losses(coarse_meshes, meshes, coarse_alpha, alpha, masks, coarse_rec, rec, imgs, coarse_rec_part, rec_part, R, T, average_txt=average_txt)
        losses, select_idx = self.compute_losses(coarse_meshes, meshes, coarse_alpha, alpha, masks, coarse_rec, rec, imgs, coarse_rec_part, rec_part, parts, R, T, average_txt=average_txt)
    ...
    def predict_mesh_pose_bkg(self, imgs, average_txt=False):
        if self.shared_encoder:
            ...
            #refined_meshes = self.predict_refined_meshes(meshes, features, local_features)
            #refined_part_meshes = self.predict_part(refined_meshes, features)
    ...
    """
    def predict_refined_meshes(self, meshes, features_sh, local_features):
        features = self.transformer_part(local_features)
        verts = meshes.get_mesh_verts_faces(0)[0]
        meshes.offset_verts_(self.predict_disp_verts(verts,features,features_sh))
        return meshes
    """
    ...
    def predict_disp_verts(self,verts,features,features_seg):
        disp_verts = []
    ...
    def compute_losses(self, meshes, refined_meshes, alpha, refined_alpha, masks, rec, refined_rec, imgs, rec_part, refined_rec_part, parts, R, T, average_txt=False):
    #def compute_losses(self, meshes, refined_meshes, alpha, refined_alpha, masks, rec, refined_rec, imgs, rec_part, refined_rec_part, R, T, average_txt=False):
        ...
        if 'rgb' in losses:
            losses['rgb'] = self.loss_weights['rgb'] * self.criterion(rec, imgs).flatten(1).mean(1) + self.loss_weights['rgb'] * self.criterion(refined_rec, imgs).flatten(1).mean(1)
        if 'mask' in losses:
        ...
        if "parts" in losses:
            #losses['parts'] = self.loss_weights['parts'] * self.segment_loss(rec_part,parts).flatten(1).mean(1)
        ...
        if 'perceptual' in losses:
            losses['perceptual'] = self.loss_weights['perceptual'] * self.perceptual_loss(rec*alpha,imgs*masks) + self.loss_weights['perceptual'] * self.perceptual_loss(refined_rec*refined_alpha, imgs*masks)

        # Mesh regularization
        if update_3d:
            if 'normal' in losses:
                losses['normal'] = self.loss_weights['normal'] * normal_consistency(meshes) + self.loss_weights['normal'] * normal_consistency(refined_meshes)
            if 'laplacian' in losses:
                losses['laplacian'] = self.loss_weights['laplacian'] * laplacian_smoothing(meshes, method='uniform') + self.loss_weights['laplacian'] * laplacian_smoothing(refined_meshes,method='uniform')
        ...
        if update_3d and 'neighbor' in losses and (self.tx_code_size > 0 and self.sh_code_size > 0):
            ...
            with torch.no_grad():
                # we recompute parameters with the current network state
                if self.shared_encoder:
                    sh1_features = self.encoder(sh1_imgs)
                    local1_features = self.local_encoder(sh1_imgs)
                    #features_tx = self.transformer_txt(local_features)
                    m = self.predict_meshes(sh1_features, local1_features, average_txt=average_txt)
                    v,f = m.get_mesh_verts_faces(0)
                    sh1_tx = self.predict_textures(f, local1_features, average_txt)
                    sh1_S = self.predict_scales(sh1_features)[:, None]
                    sh1_R, sh1_T = self.predict_poses(sh1_features)
                    sh1_bg = self.predict_background(sh1_features) if self.pred_background else None
                else:
                    m = self.predict_verts(self.encoder_sh(sh1_imgs))
                    local1_features = self.local_encoder(sh1_imgs)
                    #features_tx = self.transformer_txt(local_features)
                    v,f = m.get_mesh_verts_faces(0)
                    sh1_tx = self.predict_textures(f, local1_features, average_txt)
                    sh1_S = self.predict_scales(self.encoder_sc(sh1_imgs))[:, None]
                    sh1_R, sh1_T = self.predict_poses(self.encoder_pose(sh1_imgs))
                    sh1_bg = self.predict_background(self.encoder_bg(sh1_imgs)) if self.pred_background else None
            sh1_mesh = Meshes((verts / scales) * sh1_S, faces, sh1_tx)
            nbr_list.append([sh1_mesh, sh1_R, sh1_T, sh1_bg, sh1_imgs, sh1_masks])

            with torch.no_grad():
                # we recompute parameters with the current network state
                if self.shared_encoder:
                    sh2_features = self.encoder(sh2_imgs)
                    local2_features = self.local_encoder(sh2_imgs)
                    #features_tx = self.transformer_txt(local_features)
                    m = self.predict_meshes(sh2_features, local2_features, average_txt=average_txt)
                    v,f = m.get_mesh_verts_faces(0)
                    sh2_tx = self.predict_textures(f, local2_features, average_txt)
                    sh2_S = self.predict_scales(sh2_features)[:, None]
                    sh2_R, sh2_T = self.predict_poses(sh2_features)
                    sh2_bg = self.predict_background(sh2_features) if self.pred_background else None
                else:
                    m = self.predict_verts(self.encoder_sh(sh2_imgs))
                    local2_features = self.local_encoder(sh2_imgs)
                    #features_tx = self.transformer_txt(local_features)
                    v,f = m.get_mesh_verts_faces(0)
                    sh2_tx = self.predict_textures(f, local_features, average_txt)
                    sh2_S = self.predict_scales(self.encoder_sc(sh2_imgs))[:, None]
                    sh2_R, sh2_T = self.predict_poses(self.encoder_pose(sh2_imgs))
                    sh2_bg = self.predict_background(self.encoder_bg(sh2_imgs)) if self.pred_background else None
            sh2_mesh = Meshes((verts / scales) * sh2_S, faces, sh2_tx)
            nbr_list.append([sh2_mesh, sh2_R, sh2_T, sh2_bg, sh2_imgs, sh2_masks])
            
            with torch.no_grad():
                # we recompute parameters with the current network state
                if self.shared_encoder:
                    sh3_features = self.encoder(sh3_imgs)
                    local3_features = self.local_encoder(sh3_imgs)
                    #features_tx = self.transformer_txt(local_features)
                    m = self.predict_meshes(sh3_features, local3_features, average_txt=average_txt)
                    v,f = m.get_mesh_verts_faces(0)
                    sh3_tx = self.predict_textures(f, local3_features, average_txt)
                    sh3_S = self.predict_scales(sh3_features)[:, None]
                    sh3_R, sh3_T = self.predict_poses(sh3_features)
                    sh3_bg = self.predict_background(sh3_features) if self.pred_background else None
                else:
                    m = self.predict_verts(self.encoder_sh(sh3_imgs))
                    local3_features = self.local_encoder(sh3_imgs)
                    #features_tx = self.transformer_txt(local_features)
                    v,f = m.get_mesh_verts_faces(0)
                    sh3_tx = self.predict_textures(f, local3_features, average_txt)
                    sh3_S = self.predict_scales(self.encoder_sc(sh3_imgs))[:, None]
                    sh3_R, sh3_T = self.predict_poses(self.encoder_pose(sh3_imgs))
                    sh3_bg = self.predict_background(self.encoder_bg(sh3_imgs)) if self.pred_background else None
            sh3_mesh = Meshes((verts / scales) * sh3_S, faces, sh3_tx)
            nbr_list.append([sh3_mesh, sh3_R, sh3_T, sh3_bg, sh3_imgs, sh3_masks])
            
            with torch.no_grad():
                # we recompute parameters with the current network state
                if self.shared_encoder:
                    sh4_features = self.encoder(sh4_imgs)
                    local4_features = self.local_encoder(sh4_imgs)
                    #features_tx = self.transformer_txt(local_features)
                    m = self.predict_meshes(sh4_features, local4_features, average_txt=average_txt)
                    v,f = m.get_mesh_verts_faces(0)
                    sh4_tx = self.predict_textures(f, local4_features, average_txt)
                    sh4_S = self.predict_scales(sh4_features)[:, None]
                    sh4_R, sh4_T = self.predict_poses(sh4_features)
                    sh4_bg = self.predict_background(sh4_features) if self.pred_background else None
                else:
                    m = self.predict_verts(self.encoder_sh(sh4_imgs))
                    local4_features = self.local_encoder(sh4_imgs)
                    #features_tx = self.transformer_txt(local_features)
                    v,f = m.get_mesh_verts_faces(0)
                    sh4_tx = self.predict_textures(f, local4_features, average_txt)
                    sh4_S = self.predict_scales(self.encoder_sc(sh4_imgs))[:, None]
                    sh4_R, sh4_T = self.predict_poses(self.encoder_pose(sh4_imgs))
                    sh4_bg = self.predict_background(self.encoder_bg(sh4_imgs)) if self.pred_background else None
            sh4_mesh = Meshes((verts / scales) * sh4_S, faces, sh4_tx)
            nbr_list.append([sh4_mesh, sh4_R, sh4_T, sh4_bg, sh4_imgs, sh4_masks])
            """
            ...
            with torch.no_grad():
                ...
                if self.shared_encoder:
                    ...
                    v,f = m.get_mesh_verts_faces(0)
                    tx_verts = v + self.predict_disp_verts(v,features,tx_feat_sh).view(B, -1, 3)
    ...
    def quantitative_eval(self, loader, device, evaluator=None):
        ...
        if loader.dataset.name in ['cub_200']:
            ...
            for inp, _ in loader:
                ...
                _,_,meshes, meshes_part, (R, T), bkgs = self.predict_mesh_pose_bkg(inp['imgs'].to(device))
    ...
    def qualitative_eval(self, loader, device, path=None, N=32):
        ...
        for j, (inp, _) in enumerate(loader):
            ...
            _,_,meshes, part_meshes, (R, T), bkgs = self.predict_mesh_pose_bkg(imgs)
