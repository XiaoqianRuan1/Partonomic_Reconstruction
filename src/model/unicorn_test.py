...
class Unicorn(nn.Module):
    ...
    def tx_features(self):
        return self.local_encoder.out_ch if self.shared_encoder else self.encoder_tx.out_ch
    ...
    @torch.no_grad()
    def check_ground_truth(self, loader, device, file_path, path=None, N=3000):
        path = path or Path('.')
        self.eval()
        ...
        for j, (inp, _) in enumerate(loader):
            ...
            part, _ = parts.permute(0,3,1,2).split([N_K,1],dim=1)
            part_ground = part * masks
            B, NV = len(imgs), 50
            d, e = self.T_init[-1], np.mean(self.elev_range)
            for k in range(B):
                i = str(j*B+k).zfill(n_zeros)
                convert_to_img(imgs[k]).save(path / f'{i}_inpraw.png')
                convert_to_img(masks[k]).save(path / f'{i}_mask.png')
                ...
                self.convert_labels_to_colors(part_ground[k],os.path.join(path,f'{i}_part.png'))
                self.convert_labels_to_colors(part[k],os.path.join(path,f'{i}_part_original.png'))

    @torch.no_grad()
    def evaluate_training(self, loader, device, file_path, path=None, N=1000):
        path = path or Path('.')
        self.eval()
        self.save_prototype(path / 'model')
        n_zeros, NI = int(np.log10(N - 1)) + 1, max(N // loader.batch_size, 1)
        for j, (inp, _) in enumerate(loader):
            ...
            part_pred = self.part_renderer(part_meshes, R, T)
            part_rec,_ = part_pred.split([N_K,1],dim=1)
            part_ground = part * masks
            imgs = imgs*masks
            part_loss = self.segment_loss(part_pred,parts).flatten(1).mean(1)
            f = open(file_path,"a")
            if bkgs is not None:
    ...
    def check_part_image(self, loader, device, path=None, N=32):
        ...
        for j, (inp, _) in enumerate(loader):
            ...
            for k in range(B):
                ...
                save_mesh_as_obj(mcenter, path / f'{i}_mesh_part_weights.obj')
