...
class Unicorn(nn.Module):
    ...
    def _init_loss(self, **kwargs):
        ...
        loss_weights = {
            ...
            'parts': kwargs.pop('part_weight', 15.0),
    ...
    def forward(self, inp, debug=False):
        ...
        self._debug = debug
        B = len(inp[0]['imgs']) + len(inp[1]['imgs'])
        imgs = torch.tensor(torch.cat((inp[0]['imgs'],inp[1]['imgs']))).cuda()
        K = self.n_candidates
        """
        imgs, K, B = inp['imgs'], self.n_candidates, len(inp['imgs'])
        masks = inp['masks']
        if "parts" in inp.keys():
            parts = inp['parts']
        else:
            parts = None
        """
        masks = torch.tensor(torch.cat((inp[0]['masks'],inp[1]['masks']))).cuda()
        parts = inp[0]['parts'].cuda()
        B_part = len(parts)
        ...
        losses, select_idx = self.compute_losses(meshes, alpha, masks, rec, imgs, rec_part[:B_part*K], parts, R, T, average_txt=average_txt)
    ...
    def segment_loss(self,part_pred,part_ground):
        ...
        loss = self.criterion(part_fgs, ground_fgs.permute(0,3,1,2))
        return loss
    ...
    def compute_losses(self, meshes, alpha, masks, rec, imgs, rec_part, parts, R, T, average_txt=False):
        ...
        if "parts" in losses:
            loss_part = self.loss_weights['parts'] * self.segment_loss(rec_part,parts).flatten(1).mean(1)
            losses['parts'] = torch.cat((loss_part,torch.zeros([(B-B_part)*K]).cuda()))
        ...
        if update_3d and 'neighbor' in losses and (self.tx_code_size > 0 and self.sh_code_size > 0):
            ...
            tx_mesh = Meshes(v * tx_S, faces, textures)
    ...
    def qualitative_eval(self, loader, device, path=None, N=32):
        ...
        for j, (inp, _) in enumerate(loader):
            ...
            for k in range(B):
                ...
                #self.convert_labels_to_colors(parts[k],os.path.join(path,f'{i}_part.png'))
