...
class Unicorn(nn.Module):
    ...
    def forward(self, inp, debug=False):
        ...
        parts_to_render = repeat(meshes_part, len(T) // len(meshes_part))
        fgs_part, _ = self.part_renderer(parts_to_render, R, T).split([3, 1], dim=1)
        rec_part = fgs_part * alpha + (1 - alpha)
    ...
    def predict_mesh_pose_bkg(self, imgs, average_txt=False):
        if self.shared_encoder:
            ...
            colors = self.predict_part(meshes,features_sh)
            verts, faces = meshes.verts_padded(), meshes.faces_padded()
            part_meshes = Meshes(verts=verts,faces=faces,textures=colors)
    ...
    def predict_part(self,meshes,features):
        ...
        return TexturesVertex(verts_features=weighted_colors)
    ...
    def qualitative_eval(self, loader, device, path=None, N=32):
        ...
        for j, (inp, _) in enumerate(loader):
            ...
            part_rec, _ = self.part_renderer(part_meshes, R, T).split([3, 1], dim=1)
            part_rec = part_rec * alpha + (1 - alpha)
            if bkgs is not None:
                rec = rec * alpha + (1 - alpha) * bkgs
                part_rec = part_rec * alpha + (1 - alpha) * bkgs
            B, NV = len(imgs), 50
