...
class Unicorn(nn.Module):
    ...
    #def compute_losses(self, meshes, alpha, masks, rec, imgs, R, T, average_txt=False):
        ...
        if 'perceptual' in losses:
            losses['perceptual'] = self.loss_weights['perceptual'] * self.perceptual_loss(rec*alpha,imgs*masks)
        ...
        if update_3d and 'neighbor' in losses and (self.tx_code_size > 0 and self.sh_code_size > 0):
            ...
            for nbr_inp in nbr_list:
                ...
                if 'rgb' in losses:
                    loss += self.loss_weights['rgb'] * self.criterion(rec_sw,imgs*masks).flatten(1).mean(1)
                if 'perceptual' in losses:
                    loss += self.loss_weights['perceptual'] * self.perceptual_loss(rec_sw, imgs*masks)
                if 'mask' in losses:
    ...
    def quantitative_eval(self, loader, device, evaluator=None):
        self.eval()
        if loader.dataset.name in ['cub_200','cub_test']:
