...
class WeightsField(nn.Module):
    ...
    def __init__(self, n_units=N_UNITS, n_layers=N_LAYERS, latent_size=None, in_ch=3,
                 out_ch=N_K, zero_last_init=False, with_norm=False, dropout=False, bias_last=True):
        super().__init__()
...
class ProgressiveWeightsField(nn.Module):
    def __init__(self, inp_dim, name, powers, zero_last_init=True, milestones=None, **kwargs):
        ...
        self.field = WeightsField(NU, NL, latent_size=self.latent_size, zero_last_init=zero_last_init, bias_last=bias_last)
