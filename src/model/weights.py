...
class WeightsField(nn.Module):
    """Corresponds to a field modeled by a coordinate-based MLPs"""
    def __init__(self, out_ch, n_units=N_UNITS, n_layers=N_LAYERS, latent_size=None, in_ch=3, zero_last_init=False, with_norm=False, dropout=False, bias_last=True):
        super().__init__()
        NU, NL = n_units, n_layers
        if latent_size is not None:
            self.linear_x = nn.Linear(in_ch, NU) # input channels (3,512)
            self.linear_z = nn.Linear(latent_size, NU) # the hidden units (512, 512)
            if with_norm:
...
class ProgressiveWeightsField(nn.Module):
    def __init__(self, out_ch, inp_dim, name, powers, zero_last_init=True, milestones=None, **kwargs):
        super().__init__()
        ...
        self.field = WeightsField(out_ch, NU, NL, latent_size=self.latent_size, zero_last_init=zero_last_init, bias_last=bias_last)
