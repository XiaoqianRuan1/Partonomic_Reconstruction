...
from .attention import DETR, Transformer
from .weights import ProgressiveWeightsField
from .encoder import Encoder,PartEncoder
...
class Unicorn(nn.Module):
    ...
    @property
    def n_features(self):
        return self.encoder.out_ch if self.shared_encoder else self.encoder_tx.out_ch
        
    @property
    def deform_features(self):
        """
        original encoder_sh, and change it as encoder_tx;
        """
        return self.encoder_part.transformer.d_model
    ...
    def _init_encoder(self, img_size, **kwargs):
        ...
        if self.shared_encoder:
            ...
            self.encoder_part = DETR(N_K,img_size,**kwargs)
        else:
    ...
    def _init_meshes(self, **kwargs):
        ...
        self.deform_field1 = ProgressiveField(inp_dim=self.deform_features, name='deformation1',**dfield_kwargs)
        self.deform_field2 = ProgressiveField(inp_dim=self.deform_features, name='deformation2',**dfield_kwargs)
        self.deform_field3 = ProgressiveField(inp_dim=self.deform_features, name='deformation3',**dfield_kwargs)
        self.deform_field4 = ProgressiveField(inp_dim=self.deform_features, name='deformation4',**dfield_kwargs)
    ...
    def _init_loss(self, **kwargs):
        kwargs = deepcopy(kwargs)
        loss_weights = {
            #'entropy': kwargs.pop('entropy_weight', 1.0),
            'rgb': kwargs.pop('rgb_weight', 1.0),
            'mask': kwargs.pop('mask_weight', 1.0),
            'part': kwargs.pop('part_weight', 5.0),
        ...
        self.nbr_memory = {k: torch.empty(0) for k in ['dh', 'sh', 'tx', 'S', 'R', 'T', 'bg', 'img', 'mask']}
    ...
    def forward(self, inp, debug=False):
        ...
        part_meshes = self.predict_part(meshes)
        part_meshes_to_render = repeat(part_meshes, len(T) // len(part_meshes))
        part_fgs, part_alpha = self.renderer(part_meshes_to_render, R, T).split([3, 1], dim=1)
        part_rec = part_fgs * part_alpha + (1 - part_alpha) * bkgs if self.pred_background else part_fgs
        losses, select_idx = self.compute_losses(meshes, alpha, masks, imgs, part_imgs, part_rec, rec, R, T, average_txt=average_txt)
    ...
    def predict_mesh_pose_bkg(self, imgs, average_txt=False):
        if self.shared_encoder:
            ...
            features_part = self.encoder_part(imgs)
            meshes = self.predict_meshes(features, features_part, average_txt=average_txt)
            ...
            meshes = self.predict_meshes(features_sh, features_part, features_sc, features_tx)
    ...
    def predict_meshes(self,features_sh, features, features_sc=None, features_tx=None, average_txt=False):
        if features_tx is None:
            features_tx = features_sh
    ...
    def predict_part(self,meshes):
        weights = self.weights
        colors = np.array(color_map[:N_K])
        colors = colors.reshape(1,N_K,3)
        colors = np.repeat(colors, len(meshes), axis=0)
        verts = meshes.get_mesh_verts_faces(0)[0]
        colors = colors.reshape(colors.shape[0], 1, colors.shape[1], colors.shape[2])
        colors = np.repeat(colors, verts.shape[0], axis=1)
        colors = torch.from_numpy(colors).to("cuda")
        #weights = (weights==weights.max(dim=2,keepdim=True)[0])
        weights = weights.unsqueeze(3)
        weighted_colors = torch.sum(weights*colors,dim=2)/255.
        meshes.textures = TexturesVertex(verts_features=weighted_colors)
        return meshes
    ...
    def predict_verts(self,features):
        ...
        vert_scales = self.predict_scales_coarse(features)[:, None]
        B = new_verts.shape[0]
        faces = faces.repeat(B,1,1)
        self.direct_verts = new_verts * vert_scales
        meshes = Meshes(self.direct_verts,faces,textures)
        subdivide = SubdivideMeshes()
    ...
    def predict_disp_verts(self,verts,features,features_seg): 
        ...
        #self.weights = self.direct_predict_weights(verts,features_seg)
        #self.weights = self.generate_part_supervision_by_angle()
    ...
    def part_weights_loss(inp):
        masks = inp['masks']
        kmeans = KMeans(n_cluster=N_K,random_state=42)
        kmeans.fit(masks)
        labels = kmeans.label_
        weights = [10.0, 10.0, 10.0, 1.0]
        for i in range(N_K):
            weighted_pixels[labels==i] = masks[labels==i]*weights[i]
    ...
    def segment_loss(self,predicts,ground_truth):
        part_ground = ground_truth['parts']
        part_pred = predicts
        self.cross_entropy_loss = nn.CrossEntropyLoss(reduction='mean')
        loss = self.cross_entropy_loss(part_pred, part_ground)
        return loss
    ...
    def compute_losses(self, meshes, alpha, masks, imgs, part_imgs, part_rec, rec, R, T, average_txt=False):
        K, B = self.n_candidates, len(imgs)
        if K > 1:
            imgs = imgs.repeat(K, 1, 1, 1)
            masks = masks.repeat(K, 1, 1, 1)
            part_imgs = part_imgs.repeat(K, 1, 1, 1)
        ...
        #losses['entropy'] = self.loss_weights['entropy'] * self.part_number_regularization().abs().mean()
        # Standard reconstrution error on RGB values
        if 'rgb' in losses:
            losses['rgb'] = 1.0 * self.loss_weights['rgb'] * self.criterion(rec, imgs).flatten(1).mean(1)
        if 'mask' in losses:
            losses['mask'] = self.loss_weights['mask'] * self.mask_loss(alpha, masks)
        if 'part' in losses:
            losses['part'] = self.loss_weights['part'] * self.criterion(part_rec, part_imgs).flatten(1).mean(1)
        # Perceptual loss
        ...
        if update_3d and 'neighbor' in losses and (self.tx_code_size > 0 and self.sh_code_size > 0):
            ...
            z_sh,z_tx = [m._latent for m in [self.direct_field, self.txt_generator]]
            ...
            for n, t in [('dh',z_sh), ('tx', z_tx), ('bg', z_bg), ('S', scales), ('R', R), ('T', T), ('img', imgs),('mask', masks)]:
            ...
            with torch.no_grad():
                ...
                sim_sh = (z_sh[None] - self.nbr_memory['dh'][:, None]).pow(2).sum(-1)
            ...
            with torch.no_grad():
                ...
                if self.shared_encoder:
                    ...
                    m = self.predict_verts(sh_features)
                    v,f = m.get_mesh_verts_faces(0)
                    sh_tx = self.predict_textures(f, sh_features, average_txt)
                    ...
                    v,f = m.get_mesh_verts_faces(0)
                    sh_tx = self.predict_textures(f, self.encoder_tx(sh_imgs), average_txt)
            ...
            with torch.no_grad():
                ...
                if self.shared_encoder:
                    ...
                    tx_feat_part = self.encoder_part(tx_imgs)
                    m = self.predict_verts(tx_features)
                    v,f = m.get_mesh_verts_faces(0)
                    tx_verts = v + self.predict_disp_verts(v,tx_feat_part,tx_features).view(B, -1, 3)
                    ...
                    v,f = m.get_mesh_verts_faces(0)
                    tx_verts = v + self.predict_disp_verts(v,tx_feat_part,tx_feat_sh).view(B, -1, 3)
            ...
            for nbr_inp in nbr_list:
                ...
                if 'rgb' in losses:
                    loss += self.loss_weights['rgb'] * self.criterion(rec_sw, imgs).flatten(1).mean(1) + self.loss_weights['rgb'] * self.mask_loss(alpha_sw, masks)
                if 'perceptual' in losses:
    ...
    @torch.no_grad()
    def color_map(self):
        viridis_cmap = plt.cm.get_cmap('viridis') # 'viridis'
        num_samples = 10
        color_samples = viridis_cmap(np.linspace(0, 1, num_samples))
        rgba_values = (color_samples[:, :3] * 255).astype(np.uint8)
        return rgba_values
    """
    ...
    def get_outputs_colors(self,colors,k):
        ...
        weighted_colors = torch.sum(weights*colors,dim=1)/256.
    ...
    def get_prototype(self):
        ...
        latent = torch.zeros(1,N_K,self.deform_features,device=verts.device)
        new_meshes = meshes.offset_verts(self.predict_disp_verts(verts, latent, latent_seg).view(-1, 3))
        return new_meshes
    ...
    @torch.no_grad()
    def check_part_image(self, loader, device, path=None, N=32):
        path = path or Path('.')
        self.eval()
        ...
        for j, (inp, _) in enumerate(loader):
            ...
            for k in range(B):
                ...
                convert_to_img(parts[k]).save(path / f'{i}_part.png')
                convert_to_img(part_rec[k]).save(path / f'{i}_part_rec.png')
                if self.pred_background:
                ...
                #colors = color_map()
                mcenter.textures = self.get_outputs_colors(color_map,k)
                save_mesh_as_gif(mcenter, path / f'{i}_meshuv_weights.gif', dist=d, elev=e, renderer=renderer)
    ...
    def qualitative_eval(self, loader, device, path=None, N=32):
        ...
        for j, (inp, _) in enumerate(loader):
            ...
            d, e = self.T_init[-1], np.mean(self.elev_range)
            """
            rgb_loss = self.criterion(rec, imgs).flatten(1).mean(1)
            part_loss = self.criterion(part_rec, parts).flatten(1).mean(1)
            mask_loss = self.mask_loss(alpha, inp['masks'].to(device))
            print(rgb_loss)
            print(mask_loss)
            print(part_loss)
            print("!!!!!!!!!!!!!!!!!!!!")
            ...
            for k in range(B):
                ...
                mcenter = normalize(meshes[k])
                part_mesh = normalize(part_meshes[k])
                save_mesh_as_gif(mcenter, path / f'{i}_meshabs.gif', n_views=NV, dist=d, elev=e, renderer=renderer)
                save_mesh_as_obj(mcenter, path / f'{i}_mesh.obj')
                mcenter.textures = self.get_synthetic_textures(colored=True)
                save_mesh_as_obj(mcenter, path / f'{i}_meshuv.obj')
                ...
                #colors = color_map()
                #mcenter.textures = self.get_outputs_colors(color_map,k)
                save_mesh_as_gif(part_mesh, path / f'{i}_meshuv_weights.gif', dist=d, elev=e, renderer=renderer)