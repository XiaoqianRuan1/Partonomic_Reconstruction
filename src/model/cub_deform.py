...
class Unicorn(nn.Module):
    ...
    def _init_meshes(self, **kwargs):
        ...
        deform_kwargs = kwargs.pop('deform_fields',{})
        assert len(kwargs) == 0
        self.deform_field1 = ProgressiveField(inp_dim=self.n_features, name='deform1', **deform_kwargs)
        self.deform_field2 = ProgressiveField(inp_dim=self.n_features, name='deform2', **deform_kwargs)
        self.deform_field3 = ProgressiveField(inp_dim=self.n_features, name='deform3', **deform_kwargs)
        self.deform_field4 = ProgressiveField(inp_dim=self.n_features, name='deform4', **deform_kwargs)
        self.deform_field5 = ProgressiveField(inp_dim=self.n_features, name='deform5', **deform_kwargs)
        self.deform_field6 = ProgressiveField(inp_dim=self.n_features, name='deform6', **deform_kwargs)
    ...
    def forward(self, inp, debug=False):
        ...
        #losses, select_idx = self.compute_losses(meshes, alpha, masks, rec, imgs, R, T, average_txt=average_txt)
    ...
    def predict_meshes(self,features_sh, features_tx, features_sc=None, average_txt=False):
        ...
        meshes.offset_verts_(self.predict_disp_verts(verts,features_sh))
    ...
    def predict_disp_verts(self,verts,features): 
        disp_verts = []
        disp_verts.append(self.deform_field1(verts,features))
        disp_verts.append(self.deform_field2(verts,features))
        disp_verts.append(self.deform_field3(verts,features))
        disp_verts.append(self.deform_field4(verts,features))
        disp_verts.append(self.deform_field5(verts,features))
        disp_verts.append(self.deform_field6(verts,features))
        disp_verts = torch.stack(disp_verts,1) # BKV3
        self.weights = self.subdivide_weights(features)
    ...
    def segment_loss(self,part_pred,part_ground):
        ...
        ground_fgs = ground_fgs.permute(0,3,1,2).to(part_fgs.device)
    ...
    #def compute_losses(self, meshes, alpha, masks, rec, imgs, R, T, average_txt=False):
        ...
        if update_3d and 'neighbor' in losses and (self.tx_code_size > 0 and self.sh_code_size > 0):
            ...
            with torch.no_grad():
                ...
                if self.shared_encoder:
                    ...
                    v,f = m.get_mesh_verts_faces(0)
                    tx_verts = v + self.predict_disp_verts(v, tx_features).view(B, -1, 3)
                    ...
                    v,f = m.get_mesh_verts_faces(0)
                    tx_verts = v + self.predict_disp_verts(v, tx_feat_sh).view(B, -1, 3)
    ...
    def step(self):
        ...
        self.deform_field5.step()
        self.deform_field6.step()
        self.txt_field.step()
    ...
    def set_cur_epoch(self, epoch):
        ...
        self.deform_field5.set_cur_milestone(epoch)
        self.deform_field6.set_cur_milestone(epoch)
    ...
    def quantitative_eval(self, loader, device, evaluator=None):
        self.eval()
        if loader.dataset.name in ['cub_test','cub_200']:
    ...
    def qualitative_eval(self, loader, device, path=None, N=32):
        ...
        for j, (inp, _) in enumerate(loader):
            ...
            #part_fore, _ = parts.permute(0,3,1,2).split([N_K,1],dim=1)
            #print(part_fore.shape)
            if bkgs is not None:
            ...
            for k in range(B):
                ...
                #save_mesh_as_obj(mcenter_part, path / f'{i}_mesh_part.obj')