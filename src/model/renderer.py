...
from pytorch3d.renderer.mesh.shader import SoftPhongShader,ShaderBase
...
class Renderer(nn.Module):
    def __init__(self, img_size, layered_shader, part_shader, **kwargs):
        super().__init__()
        ...
        #elif kwargs.get('part_shader', PART_SHADER):
        elif part_shader: 
            shader_cls = PartShader
        else:
...
def gouraud_shading_plus(meshes, fragments, lights, cameras, materials, textures):
    """
    meshes: Batch of meshes
    fragments: Fragments named tuple with the outputs of rasterization
    
    Return labels: (N,H,W,K,D)
    """
    verts = meshes.verts_packed() #(V,3)
    faces = meshes.faces_packed() #(F,3)
    vertex_normals = meshes.verts_normals_packed() # (V,3)
    #verts_labels = meshes.verts_labels_packed()    #a new attribute, (V,D)
    #verts_labels = meshes.vert_labels
    #verts_colors = meshes.textures.verts_features_packed()
    faces_verts = verts[faces]
    faces_normals = vertex_normals[faces]
    #face_labels = torch.tensor(verts_labels[faces])
    #face_labels = torch.tensor(verts_colors[faces])
    """
    ...
    colors = (ambient + diffuse) * textures + specular
    """
    return textures
    """
    labels = interpolate_face_attributes(
        fragments.pix_to_face, fragments.bary_coords, face_labels)
    return labels
    """

def sigmoid_alpha_blend_labels(labels,fragments,blend_params, clip_inside=True, debug=False):
    """
    labels: (N,H,W,K,D), labels for each of the top K faces per pixel
    fragments: the outputs of rasterization
    
    Return: images (N,H,W,D+1)
    
    N,H,W,K,D = labels.shape
    is_background = fragments.pix_to_face[...,0]<0
    pixel_labels = torch.ones((N,H,W,D+1),dtype=labels.dtype,device=labels.device)
    pixel_labels[...,:D] = labels[...,0,:]
    pixel_labels[...,D] = (is_background).type_as(pixel_labels)
    return pixel_labels.permute(0, 3, 1, 2)
    """
    N,H,W,K,D = labels.shape
    background_ = blend_params.background_color
    background = torch.zeros(D,device=labels.device)
    mask = fragments.pix_to_face >= 0  # mask for padded pixels.
    if blend_params.sigma == 0:
        alpha = (fragments.dists <= 0).float() * mask
    elif clip_inside:
        ...
        alpha = torch.sigmoid(-fragments.dists / blend_params.sigma) * mask
    occ_alpha = torch.cumprod(1.0 - alpha, dim=-1)
    labels = torch.cat([labels, background[None, None, None, None].expand(N, H, W, 1, -1)], dim=-2)
    occ_alpha = torch.cat([torch.ones(N, H, W, 1, device=labels.device), occ_alpha], dim=-1)
    alpha = torch.cat([alpha, torch.ones(N, H, W, 1, device=labels.device)], dim=-1)
    pixel_labels = torch.zeros((N,H,W,D+1),dtype=labels.dtype,device=labels.device)
    pixel_labels[...,:D] = (occ_alpha[..., None] * alpha[..., None] * labels).sum(dim=-2)
    pixel_labels[...,D] = 1 - occ_alpha[:, :, :, -1]

    if debug:
        return labels, alpha, occ_alpha, pixel_labels.permute(0, 3, 1, 2)
    else:
        return pixel_labels.permute(0, 3, 1, 2)  # BCHW

class PartShader(nn.Module):
    """
    per vertex lighting -- the lighting model is applied to the vertex labels and
    the labels are then interpolated using the barycentric coordinates to 
    obtain the labels for each pixel. The blending function hard assigns the 
    labels of the closest face for each pixel.
    ...
    def forward(self, fragments, meshes, **kwargs):
        ...
        if self.shading_type == 'raw':
            ...
            if self.shading_type != 'gouraud':
                ...
                sh_kwargs['texels'] = interpolate_face_attributes(fragments.pix_to_face, fragments.bary_coords, faces_verts_features)
            colors = gouraud_shading_plus(sh_kwargs["meshes"],sh_kwargs["fragments"],sh_kwargs["lights"],sh_kwargs["cameras"],sh_kwargs["materials"],sh_kwargs['texels'])
        #pixel_labels = gouraud_shading_plus(meshes=meshes, fragments=fragments)
        #images = sigmoid_alpha_blend_labels(pixel_labels,fragments,self.blend_params,clip_inside=self.clip_inside, debug=self.debug)
        images = sigmoid_alpha_blend_labels(colors,fragments,self.blend_params,clip_inside=self.clip_inside, debug=self.debug)
        return images

class PartShaderPlus(nn.Module):
    """
    change the part blender according to the Unicorn shader. 
    """
    def __init__(self, device='cpu', cameras=None, lights=None, materials=None, blend_params=None, clip_inside=True,
                     shading_type='phong', debug=False):
        super().__init__()
...
def render_rotated_views(mesh, img_size=256, n_views=50, elev=30, dist=2.5, R=None, T=None, bkg=None,
    ...
    if eye_light:
        ...
        if isinstance(renderer.lights, AmbientLights):
            ...
            renderer = Renderer(img_size, True, False, **kwargs)
