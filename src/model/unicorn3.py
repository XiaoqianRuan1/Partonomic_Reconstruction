...
class PartMesh(Meshes):
    ...
    def vert_labels_list(self):
        self.vert_labels = padded_to_list(self.vert_labels, split_size=self.num_labels_per_mesh)
        return self.vert_labels
    
    def verts_labels_packed(self):
        verts_labels_list = self.vert_labels_list()
        return list_to_packed(verts_labels_list)[0]
...
class Unicorn(nn.Module):
    ...
    def _init_loss(self, **kwargs):
        ...
        loss_weights = {
            ...
            #'part_mask': kwargs.pop('part_weight', 1.0),
        ...
        #self.part_loss = get_loss("l1")(reduction='none')
    ...
    def forward(self, inp, debug=False):
        ...
        masks = inp['masks']
        part_imgs = torch.tensor(inp['parts'],dtype=torch.float32)
        ...
        losses, select_idx = self.compute_losses(meshes, alpha, masks, rec_part, part_imgs, rec, imgs, R, T, average_txt=average_txt)
    ...
    def segment_loss(self,part_pred,part_ground):
        ...
        part_fgs, part_bgs = part_pred.split([4,1],dim=1)
        ground_fgs, ground_bgs = part_ground.split([4,1],dim=-1)
        #loss = torch.mean(ground_fgs*torch.log2(part_fgs.permute(0,2,3,1)+0.001))
        loss = self.criterion(part_fgs.permute(0,2,3,1), ground_fgs)
        return loss
    ...
    def compute_losses(self, meshes, alpha, masks, part_rec, part_imgs, rec, imgs, R, T, average_txt=False):
        ...
        if 'part' in losses:
            losses['part'] = self.loss_weights['part'] * self.segment_loss(part_rec,part_imgs).flatten(1).mean(1)
    ...
    def qualitative_eval(self, loader, device, path=None, N=32):
        ...
        for j, (inp, _) in enumerate(loader):
            ...
            #parts,_ = inp['parts'].to(device).split([4,1],dim=-1)
            ...
            part_rec, part_alpha = self.part_renderer(part_meshes, R, T).split([4, 1], dim=1)
            if bkgs is not None:
