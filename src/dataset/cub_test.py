...
class CUBPart(TorchDataset):
    ...
    def __init__(self, split, img_size, **kwargs):
        ...
        self.split = split
        try:
            self.data_path = path_exists(os.path.join(DATASETS_PATH,self.name,'images'))
    ...
    def __getitem__(self, idx):
        ...
        #image = img * mask + torch.ones_like(img) * (1 - mask)
        img = self.transform(img)
        mask = self.transform_mask(mask)
        image = img * mask + torch.ones_like(img) * (1 - mask)
        poses = torch.cat([torch.eye(3), torch.Tensor([[0], [0], [2.732]])], dim=1)
        return {'imgs': image, "original":img, 'masks': mask, 'poses': poses}, {'kps': keypoints}
