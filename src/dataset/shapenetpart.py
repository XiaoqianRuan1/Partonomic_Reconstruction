...
class ShapeNetPartNormal(Dataset):
    ...
    cls_parts = {'earphone': [16, 17, 18], 'motorbike': [30, 31, 32, 33, 34, 35], 'rocket': [41, 42, 43], 'car': [8, 9, 10, 11], 'laptop': [28, 29], 'cap': [6, 7], 'skateboard': [44, 45, 46], 'mug': [36, 37], 'guitar': [19, 20, 21], 'bag': [4, 5], 'lamp': [24, 25, 26, 27], 'table': [47, 48, 49],'airplane': [0, 1, 2, 3], 'pistol': [38, 39, 40], 'chair': [12, 13, 14, 15], 'knife': [22, 23]}
    cls2parts = []
...
def mesh_from_pointcloud(data):
    pointcloud = data['pos']
    colors = np.array(vis_3d.color_map)[data['y']]
    texture = TexturesVertex(verts_features=torch.from_numpy(colors).unsqueeze(0))
    tri = Delaunay(pointcloud[:,:2])
    vertices = torch.tensor(pointcloud, dtype=torch.float32)
    faces = torch.tensor(tri.simplices, dtype=torch.long)
    mesh = Meshes(verts=[vertices], faces=[faces],textures=texture)
    return mesh
...
if __name__ == '__main__':
    ...
    for data in train:
        old_data = copy.deepcopy(data)
        colors = np.array(vis_3d.color_map)[data['y']]
        #vis_3d.vis_multi_points([data['pos'][:, :3]], colors=[colors/255.])
        mesh = mesh_from_pointcloud(data)