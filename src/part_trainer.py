...
class Trainer:
    ...
    def __init__(self, cfg, run_dir, gpu=None, rank=None, world_size=None):
        ...
        #self.val_scores = MeshEvaluator(['chamfer-L1', 'chamfer-L1-ICP'], self.run_dir / 'val_scores.tsv',fast_cpu=True, append=append)
        ...
        self.file_path = os.path.join(RUNS_PATH / self.dataset_name / self.run_dir, "training_loss.txt")
    ...
    def run(self):
        ...
        self.model.check_ground_truth(self.train_loader,self.device,self.file_path, path=out)
        print(aa)
        """
    ...
    def run_val_and_log(self, it, epoch, batch):
        ...
        scores = model.quantitative_eval(self.test_loader, self.device, evaluator="part")
        #scores = self.model.quantitative_eval(self.val_loader, self.device)
        print_log(LOG_FMT(epoch, self.n_epoches, batch, self.n_batches,
                          "test_scores: " + ", ".join(["{}={:.4f}".format(k, v) for k, v in scores.items()])))
        self.visualizer.upload_lineplot(it, list(scores.items()), title='test_scores')
    ...
    def evaluate(self):
        self.model.eval()
        # quantitative
        scores = self.model.quantitative_eval(self.test_loader, self.device, evaluator="part")
